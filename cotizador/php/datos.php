...
if ($resumen !== '') {
    ...
    $consulta = "INSERT INTO cotizacion(pan, queso, papas, aderezos, picante, carnes, vegetales, extras, hora, fecha, subtotal, iva, total, resumen) values ('$TPan', '$TQueso', '$TPapa', '$TAderezo', '$TPicante', '$TCarne', '$TVegetal', '$TExtra', '$hora', '$fecha', '$subtotal', '$iva', '$total', '$resumen')";

    $res=mysqli_query($conn, $consulta);

    echo '<script language="javascript">alert("¡COTIZACION GUARDADA!"); window.location = "../cotizador.html" </script>';

} else {

    echo '<script language="javascript">alert("¡TIENES QUE COTIZAR UNA TORTA!"); window.location = "../cotizador.html" </script>';
